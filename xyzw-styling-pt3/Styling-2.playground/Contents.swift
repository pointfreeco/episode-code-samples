...
extension CGFloat {
  ...
  static func pf_grid(_ env: UITraitEnvironment, _ n: Int) -> CGFloat {
    return CGFloat(n) * (env.traitCollection.horizontalSizeClass == .regular ? 8 : 4)
  }
}
 
//((Root, Value) -> Value) -> (Root) -> Root

typealias ReferenceMutableSetter<S, A> = (@escaping (inout A) -> Void) -> (S) -> Void

func filter<Root, Value>(
  _ p: @escaping (Root) -> Bool,
  _ setter: @escaping ReferenceMutableSetter<Root, Value>
  )
  -> ReferenceMutableSetter<Root, Value> {

    return { update in
      return { (root: Root) in
        p(root) ? setter(update)(root) : ()
      }
    }
}
...
func foo<Root, Value>(
  ...
  _ f: @escaping (Root, inout Value) -> Void
  )
  -> (Root)
  -> Void {

    return { root in
      f(root, &root[keyPath: kp])
    }
}

func mver<Root, Value>(
  _ kp: ReferenceWritableKeyPath<Root, Value>,
  when: @escaping (Root) -> Bool,
  _ f: @escaping (inout Value) -> Void,
  otherwise: @escaping (inout Value) -> Void
  )
  -> (Root)
  -> Void {
...
//((inout CoreGraphics.CGFloat) -> ()) -> (__ObjC.UIStackView) -> ()

//filter({ $0.traitCollection.horizontalSizeClass == .compact }, mprop(\UIStackView.spacing)) { $0 = .pf_grid(3) }
...
let baseStackViewSpacing = (filter({ $0.traitCollection.horizontalSizeClass == .compact }, mprop(\UIStackView.spacing))) { $0 = .pf_grid(3) }
...
let baseStackViewStyle = concat(
  ...
  foo(\UIStackView.spacing) { v, s in
    s = .pf_grid(v.traitCollection.horizontalSizeClass == .compact ? 3 : 4)
  },


  mver(
    \UIStackView.spacing,
    when: { $0.traitCollection.horizontalSizeClass == .compact },
    { $0 = .pf_grid(3) },
    otherwise: { $0 = .pf_grid(4) }
  ),
