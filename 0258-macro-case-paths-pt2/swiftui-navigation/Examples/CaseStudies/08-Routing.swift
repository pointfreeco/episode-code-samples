...
extension Binding {
  subscript<Enum, Case>(
    dynamicMember keyPath: CaseKeyPath<Enum, Case>
  ) -> Binding<Case?>
  where Value == Enum?, Enum: CasePathable {
    Binding<Case?>(
      get: {
        self.wrappedValue
          .flatMap(Enum.cases[keyPath: keyPath].extract(from:))
      },
      set: {
        self.wrappedValue = $0.map(Enum.cases[keyPath: keyPath].embed)
      }
    )
  }
}
