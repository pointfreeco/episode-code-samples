...
class CounterModel: HashableObject {
  ...
  var count = 0 {
    didSet {
      isTextFocused = !count.isMultiple(of: 2)
    }
  }
...
//        Button("Increment") { model.incrementButtonTapped() }
        ...
        Stepper(
          "\(model.count)",
          value: $model.count[flaggedToDouble: isFlagged]
//            Binding<Double>(
//            get: {
//              isFlagged ? Double(model.count) : 0
//            },
//            set: { newValue, transaction in
//              model.count = isFlagged ? Int(newValue) : 0
//            }
//          )
            //$model.count.toDouble
        )
...
//    let counter = UIStepper(frame: .zero, primaryAction: UIAction { [weak self] action in
//      self?.model.count = Int((action.sender as! UIStepper).value)
//    })
//    observe { [weak self] in
//      guard let self else { return }
//      counter.value = Double(model.count)
//    }
...
extension Int {
  ...
  subscript(flaggedToDouble isFlagged: Bool) -> Double {
    get { isFlagged ? Double(self) : 0 }
    set {
      self = isFlagged ? Int(newValue) : 0
    }
  }

  func foo() {
    let tmp1 = \Int[flaggedToDouble: true]
    let tmp2 = \Int[flaggedToDouble: false]
  }
}
