...
public class UINavigationStackController: UINavigationController {
  ...
  public override func viewDidLoad() {
    ...
    observe { [weak self] in
      ...
      let difference = newPath.map { $0 as! AnyHashable }
        .difference(from: self.viewControllers.compactMap(\.navigationID))

      if difference.isEmpty {
        return
      } else if difference.count == 1,
        case let .insert(newPath.count, navigationID, nil) = difference.first,
        let viewController = self.viewController(for: navigationID)
      {
  ...
  fileprivate func viewController(for navigationID: AnyHashable) -> UIViewController? {
    ...
    if #available(iOS 17, *) {
      viewController.traitOverrides.dismiss = UIDismissAction { [weak self, weak viewController] in
        guard let self, let viewController else { return }
        self.popFromViewController(viewController, animated: UIView.areAnimationsEnabled)
      }
    }
  ...
  private final class PathDelegate: NSObject, UINavigationControllerDelegate {
    ...
    func navigationController(
      ...
      base?.navigationController?(navigationController, didShow: viewController, animated: animated)
    }
    ...
    func navigationControllerPreferredInterfaceOrientationForPresentation(
      ...
      base?.navigationControllerPreferredInterfaceOrientationForPresentation?(navigationController)
...
extension UIViewController {
  fileprivate var navigationID: AnyHashable? {
    ...
    set {
      objc_setAssociatedObject(self, navigationIDKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
    }
  }
}
