...
final class MapCameraController {
    ...
    init(modelContext: ModelContext) {
        ...
        token = withContinuousObservation(options: [.didSet]) { [resultsObserver] event in
            withAnimation {
    ...
    private func calculateCameraPosition() -> MapCameraPosition {
        let coordinates = resultsObserver.results.compactMap { $0.location?.coordinate }
        ...
        for coord in coordinates.dropFirst() {
            latRange = min(latRange.lowerBound, coord.latitude)...max(latRange.upperBound, coord.latitude)
            lngRange = min(lngRange.lowerBound, coord.longitude)...max(lngRange.upperBound, coord.longitude)
        }
        ...
        let span = MKCoordinateSpan(
            latitudeDelta: max((latRange.upperBound - latRange.lowerBound) * 1.3, 0.01),
            longitudeDelta: max((lngRange.upperBound - lngRange.lowerBound) * 1.3, 0.01)
        )
