...
struct TripListView: View {
    ...
    init(selection: Binding<Trip?>, segment: Binding<ContentView.Segment>, tripCount: Binding<Int>,
        ...
        let filter = Self.buildPredicate(segment: segment.wrappedValue, searchText: searchText)
        ...
        List(selection: $selection) {
            if _trips.sections.isEmpty {
                ForEach(trips) { trip in
                    TripListRow(trip: trip, isUnread: unreadTripIdentifiers.contains(trip.persistentModelID))
                }
            } else {
    ...
    private static func buildPredicate(segment: ContentView.Segment, searchText: String) -> Predicate<Trip> {
        ...
        return #Predicate<Trip> { segmentPredicate.evaluate($0) && searchPredicate.evaluate($0) }
    }
}
