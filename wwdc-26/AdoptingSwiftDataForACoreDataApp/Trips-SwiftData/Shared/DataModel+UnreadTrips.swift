...
extension DataModel {
  ...
  var unreadTripIdentifiersInUserDefaults: [PersistentIdentifier] {
    guard
      let data = UserDefaults.standard.data(
        forKey: UserDefaultsKey.unreadTripIdentifiers
      )
    else {
      return []
    }
    let tripIdentifers = try? JSONDecoder().decode(
  ...
  func setUnreadTripIdentifiersInUserDefaults(
  ...
  private func findUnreadTrips() -> Set<Trip> {
    ...
    if let data = tokenData {
      historyToken = try? JSONDecoder().decode(
        DefaultHistoryToken.self,
        from: data
      )
    }
    let transactions = findTransactions(
      after: historyToken,
  ...
  private func findTrips(in transactions: [DefaultHistoryTransaction]) -> (
    ...
    for transaction in transactions {
      ...
      where isLivingAccommodationChange(change: change) {
        /**
