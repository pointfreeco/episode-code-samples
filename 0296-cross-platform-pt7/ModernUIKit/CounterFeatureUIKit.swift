...
final class CounterViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    let savedFactsHeaderLabel = UILabel()
    savedFactsHeaderLabel.text = "Saved facts"
    savedFactsHeaderLabel.font = .preferredFont(forTextStyle: .title1)

    let savedFactsStack = UIStackView()
    savedFactsStack.axis = .vertical
    ...
    observe { [weak self] in
      guard let self else { return }
      savedFactsHeaderLabel.isHidden = model.savedFacts.isEmpty
      savedFactsStack.arrangedSubviews.forEach { $0.removeFromSuperview() }
      for fact in model.savedFacts {
        let factLabel = UILabel()
        factLabel.numberOfLines = 0
        factLabel.text = fact

        let deleteButton = UIButton(type: .system, primaryAction: UIAction { [weak self] _ in
        ...
        factStack.axis = .horizontal
        factStack.distribution = .equalSpacing
        savedFactsStack.addArrangedSubview(factStack)
      }
    }
